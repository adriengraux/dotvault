...
function init (argv) {
  ...
  if (fs.existsSync(commandVaultFile) && !commandForce) {
    ...
    console.error('specify vault whit -v option or force init with -f option')
    process.exit(1)
  }
  ...
  console.log(`Set your secrets in ${vault.envFile} into \n \t${vault.pattern.begin}\n\t\tSECRETS HERE\n\t${vault.pattern.end}\n--> then encrypt with dotvault encrypt`)
  console.log(`You MUST commit the ${commandVaultFile} for sharing secrets with your team`)
}
