...
module.exports = {
  ...
  extractKeyIvFromKeyVault: (keyVault) => {
    const ivLength = configuration.key.ivLength
    const keyLength = configuration.key.length - ivLength

    return {
      key: keyVault.slice(0, keyLength),
      iv: keyVault.slice(keyLength, keyLength + ivLength)
    }
  },
